...
class TestPollChannelFor:
    """Tests for poll_channel_for() -- post-migration-009 legacy fallback.

    poll_channel moved to channel_schedules; poll_channel_for now only
    returns the config-level fallback.
    """

    def test_returns_config_channel(self, app, clean_all_tables):
        """poll_channel_for returns config-level channel."""
        app.config['SLACK_POLL_CHANNEL'] = '#config-channel'
        with app.app_context():
            from lunchbot.services.poll_service import poll_channel_for
            assert poll_channel_for('T_ANY') == '#config-channel'
