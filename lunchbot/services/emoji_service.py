...
def search_suggestions(emoji_entry):
    ...
    for search in emoji_entry['search_query']:
        ...
        response = places_client.find_suggestion(search)
...
def search_and_update_emoji():
    ...
    and updates their emoji field in PostgreSQL.
    """
    ...
    for emoji_entry in emojis:
        results = search_suggestions(emoji_entry)
        if results:
